...
func _multimesh_new(node, meta, meta_val) -> void:
	...
	if meta_val not in multimesh_dict:
		...
		for mat_meta in metas:
			var mat_meta_val = node.get_meta(mat_meta)
			if "material" in mat_meta:
				var material = load(mat_meta_val)
				print(mat_meta_val)
				mi_mesh.surface_set_material(0, material)
